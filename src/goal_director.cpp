...
const noexcept {
    ...
    const double distance_from_waypoint = distance(nav_point, waypoint_point);

    ROS_DEBUG_STREAM("current utm location: " << nav_point);
    ROS_DEBUG_STREAM("current waypoint location: " << waypoint_point);
